<script lang="ts">
	...
	import Modal from '../common/ModalFinance.svelte';
...
<Modal bind:show size="md">
	<div>
		<div class=" flex justify-between dark:text-gray-300 pb-0.5">
			<div class="font-medium self-center">{$i18n.t('Available Tools')}</div>
			<button
				class="self-center"
				on:click={() => {
					show = false;
		...
		{#if selectedTools.length > 0}
			{#if $toolServers.length > 0}
				<div class=" flex justify-between dark:text-gray-300 pb-1">
					<div class=" text-base self-center">{$i18n.t('Tools')}</div>
				</div>
			{/if}

			<div class="pb-3 w-full flex flex-col justify-center">
		...
		{#if $toolServers.length > 0}
			...
			<div class="pb-5 w-full flex flex-col justify-center">
