<script lang="ts">
	...
	import { WEBUI_NAME, config, settings, prompts, tools as _tools, user } from '$lib/stores';
	...
	import Tools  from '$lib/components/chat/Settings/ToolsFinanceinvest.svelte';
	import ToolServersModalFinance from '$lib/components/chat/ToolServersModalFinance.svelte';
...
<svelte:head>
	<title>
		{$i18n.t('ToolsFinance')} | {$WEBUI_NAME}
	</title>
</svelte:head>
...
{#if loaded}
	...
	{/if}
	{#if financeinvest || lifesciences}
		<Tools	saveSettings={async (updated) => {
				//await saveSettings(updated);
				toast.success($i18n.t('Settings saved successfully!'));
		}}/>
		<ToolServersModalFinance bind:show={showTools}/>		
	{/if}	
